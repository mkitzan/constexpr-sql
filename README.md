...
The following example shows usage of all class templates a user is expected to define to use the library. `sql::schema` defines a relation used in a query. `sql::index` defines how an `sql::schema` sorts its data (unsorted if unspecified). `sql::column` types are used to defines the rows in an `sql::schema`. `sql::query` wraps an SQL query and the `sql::schema` types the query will operate on. `sql::load` can be used to load data from a file into an `sql::schema`.
...
int main()
{
	authored a{ sql::load<authored, '\t'>("authored-data.tsv") };
	books b{ sql::load<books, '\t'>("books-data.tsv") };
